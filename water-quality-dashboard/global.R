...
all_westport <- read_csv(here("data", "all_westport.csv")) %>% 
  ...
  # don't have long/lat for
  filter(!site_name %in% c("Stony 1.6", "Stony 1.7"))

# geometries
westport_1 <- read_sf(here("data", "raw", "geoms", "westport_attempt1.geojson"))
pussy_willow <- read_sf(here("data", "raw", "geoms", "Pussy Willow.geojson"))
stony_brook_1 <- read_sf(here("data", "raw", "geoms", "Stony Brook 1.geojson"))
stony_brook_2 <- read_sf(here("data","raw",  "geoms", "Stony Brook 2.geojson"))
west_branch <- read_sf(here("data", "raw", "geoms", "West Branch Saugatuck River.geojson"))
greens_farms <- read_sf(here("data", "raw", "geoms", "Greens Farms Brook (Westport)-01.geojson"))


# read in sampling years
sampling_years <- read_csv(here("data", "raw", "westport_sampling_years.csv"))
...
site_order <- list(
  ...
  "Sasco Brook" = c("Great 4", "Great 3", "Great 2", "Great 1", "Sasco 9", "Hunt Club 5", "Hunt Club 4", "Hunt Club 3", "Hunt Club 2", "Hunt Club 1", "Sasco 8", "Sasco 7", "Sasco 6", "Sasco 5", "Sasco 4", "Sasco 3", "Sasco 2", "Sasco 1"),
  ...
  "Stony Brook" = c("Stony 5", "Stony 4", "Stony 3", "Stony 2", "Stony 1.5", "Stony 1.5 Downstream", "Stony 1")
)
